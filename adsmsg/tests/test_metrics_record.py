...
class TestMsg(unittest.TestCase):
    ...
    def test_simple_data(self):
        ...
        map_fields = ("rn_citations_hist",)
        m = MetricsRecord(**metrics_data)
        for key in metrics_data:
            #self.assertEqual(getattr(m, key), metrics_data[key])
            if key not in map_fields:
                self.assertAlmostEqual(getattr(m, key), metrics_data[key], msg='{} field failed'.format(key))
            else:
                sub_m = getattr(m, key)
                for sub_key in metrics_data[key]:
                    self.assertAlmostEqual(sub_m[sub_key], metrics_data[key][sub_key], msg='{} map field failed for key {}'.format(key, sub_key))
    ...
    def test_rn_citation_data(self):
        ...
        for i in range(0, len(t)):
            for key in t[i]:
                self.assertAlmostEqual(t[i][key], getattr(p[i], key), msg='rn_citation_data field {}'.format(key))
